...
def generate_kernel_inputs(
    ...
    o_proj, lse, norm_score = compute_o_lse(
        convert_to_torch(q),
        convert_to_torch(k),
        convert_to_torch(v),
        causal,
    ...
    o_proj = nl.static_cast(o_proj.float().numpy(), dtype)
    lse = lse.float().numpy()
    norm_score = norm_score.float().numpy()
    ...
    result = {
        ...
        "softmax_scale": softmax_scale,
        "sliding_window": sliding_window,
        # Cache attention scores to avoid recomputation
...
class TestAttentionBwdKernel:
    ...
    test_cases_8K = [
        (1, 32, 8, 8192, 128, nl.bfloat16, True, None, 0),   # llama3.1-8b
        ...
        (1, 32, 8, 8192, 64, nl.bfloat16, True, None, 0),    # llama3.2-1b
        (1, 32, 32, 8192, 128, nl.bfloat16, True, None, 0),  # MHA baseline
    ]
    ...
    test_cases_misc = [
        ...
        (1, 8, 2, 16384, 128, nl.bfloat16, True, None, 0),    # phi-4 (16K seqlen, reduced heads)
    ]
    ...
    def run_attention_bwd_test(
        ...
        with assert_negative_test_case(is_negative):
            ...
            q, k, v, dy = kernel_input["q_ref"], kernel_input["k_ref"], kernel_input["v_ref"], kernel_input["dy_ref"]
            golden_input = {
                "q": convert_to_torch(q),
                "k": convert_to_torch(k),
                "v": convert_to_torch(v),
                "dy": convert_to_torch(dy),
                "norm_score": convert_to_torch(kernel_input.pop("norm_score")),
                "mixed_precision": kernel_input["mixed_precision"],
                "softmax_scale": kernel_input["softmax_scale"],
                "use_causal_mask": kernel_input["use_causal_mask"],
                "sinks": convert_to_torch(kernel_input["sinks_ref"]) if num_sinks > 0 else None,
            }
            output_placeholder = {
                "out_dq_ref": np.zeros(q.shape, q.dtype),
                "out_dk_ref": np.zeros(k.shape, k.dtype),
                "out_dv_ref": np.zeros(v.shape, v.dtype),
            }
            if num_sinks > 0:
                output_placeholder["out_dsinks_ref"] = np.zeros(
                    kernel_input["sinks_ref"].shape, kernel_input["sinks_ref"].dtype
                )

            def create_golden():
                result = attention_bwd_torch_ref(**golden_input)

                if num_sinks > 0:
                    dq, dk, dv, dsinks = result
                else:
                    dq, dk, dv = result

                output = {
                    "out_dq_ref": nl.static_cast(dq.float().numpy(), q.dtype),
                    "out_dk_ref": nl.static_cast(dk.float().numpy(), k.dtype),
                    "out_dv_ref": nl.static_cast(dv.float().numpy(), v.dtype),
                }

                if num_sinks > 0:
                    output["out_dsinks_ref"] = nl.static_cast(dsinks.float().numpy(), kernel_input["sinks_ref"].dtype)

                return output

            test_manager.execute(
                KernelArgs(
                    kernel_func=attention_bwd,
