...
import numpy as np
import pytest
from nkilib_src.nkilib.experimental.subkernels.find_nonzero_indices import find_nonzero_indices


def find_nonzero_indices_reference(
    input_tensor: np.ndarray, row_start_id: int = None, n_rows: int = None
) -> dict[str, np.ndarray]:
    """
    ...
    Args:
        ...
        row_start_id: Starting column index (if specified, only process n_rows columns starting from this index)
        n_rows: Number of columns to process
...
def generate_sparse_input(T: int, E: int, top_k: int, dtype: np.dtype, seed: int = 42) -> np.ndarray:
    ...
    Args:
        T: Number of tokens (rows in output)
...
class TestFindNonzeroIndicesKernel:
    ...
    # Tests WITHOUT row_start_id (full tensor processing)
    @pytest.mark.fast
    ...
    # Tests WITH row_start_id and n_rows (subset processing)
    @pytest.mark.fast
    @pytest.mark.parametrize(
    ...
    def test_find_nonzero_indices_with_row_start_fast(
        self,
        test_manager: Orchestrator,
        T,
        E,
        ...
        """Fast tests with row_start_id and n_rows for subset processing."""
        input_tensor = generate_sparse_input(T, E, top_k=top_k, dtype=in_dtype)
        row_start_id = np.array([E_offset]).astype(np.int32)

        def golden_generator():
            return find_nonzero_indices_reference(input_tensor, row_start_id=E_offset, n_rows=E_local)
