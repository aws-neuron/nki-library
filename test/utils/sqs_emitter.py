...
class SQSEmitter(IMetricsEmitter):
    ...
    def emit(self) -> None:
        ...
        # TEMPORARY: Control individual test result emission while migrating tests
        # Set EMIT_TEST_RESULTS=true for tests migrated to coverage_parametrize/UnitTestFramework
        # Default is disabled to avoid emitting poorly-formatted params from legacy RangeTestCase tests
        # TODO: Remove this flag once all tests are migrated to new frameworks
        if os.environ.get("EMIT_TEST_RESULTS", "").lower() != "true":
            return
        ...
        try:
            ...
            self._logger.info(f"SQS Params being sent: {json.dumps(payload.get('Params', {}))}")
    ...
    def emit_run_complete(
        self, kernel_name: str, tests_passed: int, tests_total: int, tests_skipped: int = 0, tests_xfailed: int = 0
    ) -> None:
    ...
    def _build_payload(self, metrics_context) -> dict:
        ...
        return {
            "RunId": self._run_id,
            "Timestamp": datetime.now(timezone.utc).isoformat(),
            **dimensions,  # Include all dimensions (TestName, KernelName, Target, Status, etc.)
            "Params": params,
            "Metrics": metrics,
        }
