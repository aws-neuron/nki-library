...
def mla_vupmx_oproj_cte_kernel(
    ...
    Pseudocode:
        ...
        return out
    """
    _validate_mla_vupmx_oproj_inputs(out_attn_hbm, wuv_qtz_hbm, wuv_scale_hbm, wo_qtz_hbm, wo_scale_hbm)
...
def _vupmx_oproj_stage(
    ...
    in_qtz = sbm.alloc_stack(
        (_P_MAX, num_mx_k_tiles, s_per_core), dtype=nl.float8_e4m3fn_x4, buffer=nl.sbuf, name="oproj_in_qtz"
    )
    in_scale = sbm.alloc_stack(
        (_P_MAX, num_mx_k_tiles, s_per_core), dtype=nl.uint8, buffer=nl.sbuf, name="oproj_in_scale"
    )

    # V-up scratch in its OWN scope so attn_v/lat_*/wuv_* free before Phase B allocates the
    # weight slab + accumulator.
    sbm.open_scope()
    attn_v = sbm.alloc_stack((oproj_s_tile, Hdv), dtype=nl.bfloat16, buffer=nl.sbuf, name="attn_v")
    ...
    for buf_idx in range(n_lat_buf):
        lat_qtz.append(
            sbm.alloc_stack(
                (_P_MAX, num_lat_k_tiles, oproj_s_tile),
        ...
        lat_scale.append(
            sbm.alloc_stack(
                (_P_MAX, num_lat_k_tiles, oproj_s_tile), dtype=nl.uint8, buffer=nl.sbuf, name=f"vup_lat_scale_{buf_idx}"
            )
        )
        lat_in_t.append(
            sbm.alloc_stack(
                (_P_MAX, lat_stage_512, oproj_s_tile * _H_PACK),
                dtype=nl.bfloat16,
                buffer=nl.sbuf,
        ...
        lat_planes.append(
            sbm.alloc_stack(
                (_P_MAX, lat_stage_512, _H_PACK, oproj_s_tile),
                dtype=nl.bfloat16,
                buffer=nl.sbuf,
    ...
    in_t = sbm.alloc_stack(
        (_P_MAX, stage_512, oproj_s_tile * _H_PACK), dtype=nl.bfloat16, buffer=nl.sbuf, name="oproj_t"
    )
    wuv_qtz, wuv_scale = _load_mx_weights(wuv_qtz_hbm, wuv_scale_hbm, HL, d_v, sbm, name="vup_wuv")
    ...
    cross-kernel 4-pack column order _attention_stage writes (each (k512, sub)'s groups
    ...
    for chunk in range(num_lat_stage_chunks):
        ...
        nisa.quantize_mx(
            src=lat_in_t[0][0:_P_MAX, 0:lat_stage_512, 0 : oproj_s_tile * _H_PACK],
            dst=lat_qtz[0][0:_P_MAX, nl.ds(k512_start, lat_stage_512), 0:oproj_s_tile],
            dst_scale=lat_scale[0][0:_P_MAX, nl.ds(k512_start, lat_stage_512), 0:oproj_s_tile],
        )
    for s_batch in range(num_s_batches):
        ...
        if n_lat_buf > 1 and s_batch + 1 < num_s_batches:
            ...
            for chunk in range(num_lat_stage_chunks):
                ...
                nisa.quantize_mx(
                    src=lat_in_t[ld_buf][0:_P_MAX, 0:lat_stage_512, 0 : oproj_s_tile * _H_PACK],
                    dst=lat_qtz[ld_buf][0:_P_MAX, nl.ds(k512_start, lat_stage_512), 0:oproj_s_tile],
                    dst_scale=lat_scale[ld_buf][0:_P_MAX, nl.ds(k512_start, lat_stage_512), 0:oproj_s_tile],
                )
        ...
        for h in range(H):
            vup_psum = nl.ndarray(
                (oproj_s_tile, d_v), dtype=nl.bfloat16, buffer=nl.psum, address=(0, _H_PACK * PSUM_BANK_SIZE)
            )
            nisa.nc_matmul_mx(
                dst=vup_psum[0:oproj_s_tile, 0:d_v],
                stationary=lat_qtz_b[0:_P_MAX, h, nl.ds(0, oproj_s_tile)],
                moving=wuv_qtz[0:_P_MAX, h, nl.ds(0, d_v)],
                stationary_scale=lat_scale_b[0:_P_MAX, h, nl.ds(0, oproj_s_tile)],
                moving_scale=wuv_scale[0:_P_MAX, h, nl.ds(0, d_v)],
            )
            base = (h // _H_PACK) * _P_MAX + (h % _H_PACK) * (_P_MAX // _H_PACK)
            nisa.tensor_scalar(
                attn_v.ap(
                    pattern=[[Hdv, oproj_s_tile], [1, _P_MAX // _H_PACK], [sub_stride, _H_PACK]],
                    offset=base,
                ),
                vup_psum,
                op0=nl.multiply,
        ...
        for chunk in range(num_stage_chunks):
            ...
            _transpose_preswizzled_for_mx(
                attn_v, oproj_s_tile, num_mx_k_tiles, stage_512, in_t, tpsum, base_col_512=k512_start
            )
            nisa.quantize_mx(
                src=in_t[0:_P_MAX, 0:stage_512, 0 : oproj_s_tile * _H_PACK],
                dst=in_qtz[0:_P_MAX, nl.ds(k512_start, stage_512), nl.ds(s_off, oproj_s_tile)],
                dst_scale=in_scale[0:_P_MAX, nl.ds(k512_start, stage_512), nl.ds(s_off, oproj_s_tile)],
            )
    ...
    """
    out_acc = sbm.alloc_stack((oproj_s_tile, hid_per_core), dtype=nl.bfloat16, buffer=nl.sbuf, name="oproj_out_acc")
    out_sb = sbm.alloc_stack((oproj_s_tile, hid_per_core), dtype=nl.bfloat16, buffer=nl.sbuf, name="oproj_out")
    """
            ...
            for hid_idx in range(num_hid_tiles):
                ...
                for i_n in range(num_n_tiles):
                    ...
                    for k_local in range(slab_512):
                        ...
                        nisa.nc_matmul_mx(
                            dst=ps[0:oproj_s_tile, 0:n_sz],
                            stationary=in_qtz[0:_P_MAX, k_global, nl.ds(s_off, oproj_s_tile)],
                            moving=w_buf[0:_P_MAX, w_k, nl.ds(n_base, n_sz)],
                            stationary_scale=in_scale[0:_P_MAX, k_global, nl.ds(s_off, oproj_s_tile)],
                    ...
                    if slab_id == 0:
                        nisa.tensor_copy(
                            dst=out_acc[0:oproj_s_tile, nl.ds(n_base, n_sz)],
                            src=ps[0:oproj_s_tile, 0:n_sz],
                            engine=nisa.engine.vector,
                        )
                    else:
                        nisa.tensor_tensor(
                            dst=out_acc[0:oproj_s_tile, nl.ds(n_base, n_sz)],
                            data1=out_acc[0:oproj_s_tile, nl.ds(n_base, n_sz)],
                            data2=ps[0:oproj_s_tile, 0:n_sz],
                            op=nl.add,
                            engine=nisa.engine.vector,
                        )
