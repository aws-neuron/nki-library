...
def attention_mxfp8_tkg_torch_ref(
    ...
    for batch_idx in range(bs):
        ...
        mask_b = mask_np[batch_idx * num_chunks : (batch_idx + 1) * num_chunks]
        token_valid = _chunk_masks_to_token_mask(mask_b, len(table_b))
...
def _chunk_masks_to_token_mask(mask_b, num_blocks):
    """Convert per-chunk eviction-layout masks to a flat per-token validity vector.

    The chunk mask matches the kernel's score buffer layout: block g of chunk c
    lands at partition half g%2 and free offset (pair * 4 + fold) * 128, where
    fold = (g % 16) // 4 and pair = (g % 4) // 2.

    Args:
        mask_b: [num_chunks, 128, score_free] uint8 masks for one batch element.
        num_blocks: Number of KV blocks covered by the masks.

    Returns:
        [num_blocks * BLOCK_LEN] bool per-token validity.
    """
    half_p = 64
    token_valid = np.zeros(num_blocks * _BLOCK_LEN, dtype=bool)
    for block_idx in range(num_blocks):
        chunk_idx, block_in_chunk = divmod(block_idx, _BLOCKS_PER_CHUNK)
        fold_idx, block_in_fold = divmod(block_in_chunk, _BLOCKS_PER_FOLD)
        pair_idx, half_idx = divmod(block_in_fold, 2)
        f_base = (pair_idx * _BLOCKS_PER_FOLD + fold_idx) * _BLOCK_LEN
        p_row = half_idx * half_p
        token_valid[block_idx * _BLOCK_LEN : (block_idx + 1) * _BLOCK_LEN] = (
            mask_b[chunk_idx, p_row, f_base : f_base + _BLOCK_LEN] != 0
        )
    return token_valid
