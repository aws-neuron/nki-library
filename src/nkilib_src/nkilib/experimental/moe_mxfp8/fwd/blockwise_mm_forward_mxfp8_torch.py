...
def blockwise_mm_fwd_mxfp8_torch_ref(
    ...
    if no_indirect_load:
        ...
        # kernel's checkpoint_config gating and the routed path below).
        ...
        if checkpoint_config.save_scaled_intermediate:
            scaled_intermediate_checkpoint_T = np.zeros((N, I_TP, B), dtype=dtype)
            for block_idx in range(N):
                start = block_idx * B
                end = start + B
                scaled_intermediate_checkpoint_T[block_idx] = scaled[start:end, :].T
    ...
    # gate_up_activations_T ([N, 2, I_TP, B]) is always computed by the golden (it
    # is needed to derive the scaled intermediate below) but only returned as a
    # checkpoint when the save flag is set.
    if checkpoint_config.save_gate_up_proj_act:
        result["gate_up_proj_act_checkpoint_T"] = torch.from_numpy(np.ascontiguousarray(gate_up_activations_T))

    # Derive scaled_intermediate_checkpoint_T from the (already clamped) gate/up
    # checkpoint + affinity: SiLU(gate) * up * EA, transposed to [N, I_TP, B].
    # gate_up_activations_T is [N, 2, I_TP, B] with gate at [:,0], up at [:,1].
    if checkpoint_config.save_scaled_intermediate:
        tok_pos_2d = tok_pos_np.reshape(N, B)
        scaled_intermediate_checkpoint_T = np.zeros([N, I_TP, B]).astype(dtype)
        for b in range(N):
            ...
            scaled_intermediate_checkpoint_T[b] = (inter_t * ea).astype(dtype)
        result["scaled_intermediate_checkpoint_T"] = torch.from_numpy(
            np.ascontiguousarray(scaled_intermediate_checkpoint_T)
        )
