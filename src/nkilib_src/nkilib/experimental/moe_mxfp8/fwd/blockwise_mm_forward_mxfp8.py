...
def blockwise_mm_fwd_mxfp8(
    ...
    Returns:
        tuple:
          ...
          present only when its checkpoint_config flag is set):
            - gate_up_proj_act_checkpoint_T (nl.ndarray): [N, 2, I_TP, B], clamped
              gate pre-activation at [block, 0] and up at [block, 1] (B contiguous);
              present when checkpoint_config.save_gate_up_proj_act.
            - scaled_intermediate_checkpoint_T (nl.ndarray): [N, I_TP, B],
              SiLU(gate)*up*EA transposed; present when
              checkpoint_config.save_scaled_intermediate.
    """
    if skip_dma == None:
    ...
    # checkpoint is passed to the impl as None so it skips the transpose+store.
    gate_up_proj_act_checkpoint_T = None
    if checkpoint_config.save_gate_up_proj_act:
        gate_up_proj_act_checkpoint_T = nl.ndarray((N, 2, I_TP, block_size), dtype=compute_dtype, buffer=hbm_buffer)
    scaled_intermediate_checkpoint_T = None
    if checkpoint_config.save_scaled_intermediate:
        scaled_intermediate_checkpoint_T = nl.ndarray((N, I_TP, block_size), dtype=compute_dtype, buffer=hbm_buffer)
