...
from .mlp_cte_basic_utils import (
...
def perform_down_projection(
    ...
    if mlp_params.quant_params.is_quant_row() or mlp_params.quant_params.is_quant_static():
        _perform_quantized_down_projection(
            mlp_params,
            tile_info,
            constants,
            indices,
...
def _perform_quantized_down_projection(
    mlp_params: MLPParameters,
    tile_info: MLPCTEBasicTileInfo,
    constants: MLPCTEConstants,
    indices: MlpBxsIndices,
    ...
    for hidden_tile in hidden_tiles:
        ...
        for int_tile in int_doublerow_tiles:
            ...
            for bxs_subtile in TiledRange(current_bxs_tile, BXS_SUBTILE_SIZE):
                for hidden_subtile in hidden_subtiles:
                    psum_bank = (bxs_subtile.index * len(hidden_subtiles) + hidden_subtile.index) % NUM_HW_PSUM_BANKS
...
def perform_gate_projection_if_necessary(
    ...
    Args:
        ...
        gate_weight_scales_sbuf: Optional gate weight row dequant scales in SBUF
...
def perform_up_projection(
    ...
    Args:
        ...
        weights_sbuf_list: Weight buffers in SBUF
        bias_tensor_sbuf: Optional bias tensor in SBUF
        up_weight_scales_sbuf: Optional up weight row dequant scales in SBUF
...
def _project_source_tensor_tile(
    ...
    Args:
        ...
        weight_scales_sbuf: Weight scales in SBUF
    ...
    """
    if mlp_params.quant_params.is_quant_row() or mlp_params.quant_params.is_quant_static():
        _project_quantized_source_tensor_tile(
            mlp_params,
            tile_info,
            constants,
            bxs_tile_idx,
        ...
        _project_standard_source_tensor_tile(
            mlp_params,
            tile_info,
            constants,
            bxs_tile_idx,
...
def _project_standard_source_tensor_tile(
    mlp_params: MLPParameters,
    tile_info: MLPCTEBasicTileInfo,
    constants: MLPCTEConstants,
    bxs_tile_idx: int,
...
def _project_quantized_source_tensor_tile(
    mlp_params: MLPParameters,
    tile_info: MLPCTEBasicTileInfo,
    constants: MLPCTEConstants,
    bxs_tile_idx: int,
