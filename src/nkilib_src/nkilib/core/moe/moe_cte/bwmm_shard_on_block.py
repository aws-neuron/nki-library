...
def compute_same_weights_block_parallel_hbm(
    ...
    all_expert_indices = nl.ndarray(
        block_to_expert.ap(pattern=[[2 * n_tile_minus_1, tile_size], [2, n_tile_minus_1]], offset=shard_id).shape,
        dtype=nl.int32,
        buffer=nl.sbuf,
    )

    # DMA Pattern 1: Full rows - loads complete rows of expert indices for blocks
    # that fit entirely within the shard's allocation
    if num_acc_p_1 > 0:
        nisa.dma_copy(
    ...
    # DMA Pattern 2: Partial row - loads remaining expert indices that don't
    # fill a complete row due to block count not being evenly divisible
    if num_acc_p_2 > 0:
        partial_row_offset = shard_id + 2 * n_tile_minus_1 * num_full_rows
        nisa.dma_copy(
            dst=all_expert_indices[num_full_rows : num_full_rows + 1, 0:num_acc_f_2],
            src=block_to_expert.ap(
                pattern=[
                    [2 * n_tile_minus_1, 1],
                    [2, num_acc_f_2],
    ...
    if num_acc_p_1 > 0:
        nisa.dma_copy(
            ...
            src=block_to_expert.ap(
                pattern=[[2 * n_tile_minus_1, num_acc_p_1], [2, num_acc_f_1]],
                offset=shard_id + off_one_offset,
            ),
        )

    # Pattern 2: Partial row (offset shifted by off_one_offset)
    if num_acc_p_2 > 0:
        partial_row_offset_off_one = shard_id + off_one_offset + 2 * n_tile_minus_1 * num_full_rows
        nisa.dma_copy(
            dst=all_expert_indices_off_one[num_full_rows : num_full_rows + 1, 0:num_acc_f_2],
            src=block_to_expert.ap(
                pattern=[[2 * n_tile_minus_1, 1], [2, num_acc_f_2]], offset=partial_row_offset_off_one
            ),
        )
    ...
    if num_acc_p_2 > 0:
        nisa.tensor_tensor(
            data1=all_expert_indices[num_full_rows : num_full_rows + 1, 0:num_acc_f_2],
            data2=all_expert_indices_off_one[num_full_rows : num_full_rows + 1, 0:num_acc_f_2],
            op=nl.equal,
            dst=is_weight_same_as_prev[num_full_rows : num_full_rows + 1, 0:num_acc_f_2],
        )
    ...
    # Save to HBM to reduce SBUF memory pressure
    is_weight_same_as_prev_hbm = nl.ndarray((n_blocks_per_shard,), dtype=nl.uint8, buffer=nl.private_hbm)

    # Calculate access counts for HBM store
    # Mask: load_f + n_tile_minus_1 * load_p + 1 < n_blocks_per_shard
    # Equivalent: load_f + n_tile_minus_1 * load_p < n_blocks_per_shard - 1
    max_linear_index_hbm = n_blocks_per_shard - 1

    num_full_rows_hbm = min(tile_size, max(0, max_linear_index_hbm // n_tile_minus_1))
    num_acc_p_hbm_1 = num_full_rows_hbm
    num_acc_f_hbm_1 = n_tile_minus_1 if num_full_rows_hbm > 0 else 0

    if num_full_rows_hbm < tile_size and max_linear_index_hbm > 0:
        num_partial_f_hbm = max_linear_index_hbm % n_tile_minus_1
        num_acc_p_hbm_2 = 1 if num_partial_f_hbm > 0 else 0
        num_acc_f_hbm_2 = num_partial_f_hbm
    else:
        num_acc_p_hbm_2 = 0
        num_acc_f_hbm_2 = 0

    # Store 1: Single zero at index 0
    nisa.dma_copy(dst=is_weight_same_as_prev_hbm[0:1], src=zero_index[0:1, 0:1])

    # Store 2: Pattern 1 - Full rows
    if num_acc_p_hbm_1 > 0:
        nisa.dma_copy(
            dst=is_weight_same_as_prev_hbm.ap(
                pattern=[
                    [n_tile_minus_1, num_acc_p_hbm_1],  # p dimension stride
                    [1, num_acc_f_hbm_1],  # f dimension (contiguous)
                ],
                offset=1,  # Start at index 1
            ),
            src=is_weight_same_as_prev[0:num_acc_p_hbm_1, 0:num_acc_f_hbm_1],
        )

    # Store 3: Pattern 2 - Partial row
    if num_acc_p_hbm_2 > 0:
        partial_row_offset_hbm = 1 + n_tile_minus_1 * num_full_rows_hbm
        nisa.dma_copy(
            dst=is_weight_same_as_prev_hbm.ap(
                pattern=[
                    [n_tile_minus_1, 1],  # 1 row
                    [1, num_acc_f_hbm_2],  # partial columns
                ],
                offset=partial_row_offset_hbm,
            ),
            src=is_weight_same_as_prev[num_full_rows_hbm : num_full_rows_hbm + 1, 0:num_acc_f_hbm_2],
        )
