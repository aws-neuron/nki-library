...
# Valid partition counts for quantize_mx hardware constraint
_VALID_QMX_PARTITION_COUNTS = (32, 64, 96, 128)


def _pad_to_valid_qmx_partitions(n: int) -> int:
    for valid in _VALID_QMX_PARTITION_COUNTS:
        if valid >= n:
            return valid
    return 128
...
def load_gate_up_weight_scale_bias(
    ...
    This ensures alignment with down_projection_mx_shard_I which also uses tile-based I-sharding.
    ...
    Notes:
        - Uses tile-based I-sharding to align with down_projection_mx_shard_I
    ...
    if is_bias:
        ...
        if I_p_bias_in_hbm < TILE_I:
            ...
            nisa.dma_copy(src=bias_view.get_view(), dst=bias_sb[...])
...
def gate_up_projection_mx_shard_I(
    ...
    n_total_I512_tiles = div_ceil(I_local, 512)
    I_4, TILE_I = 4, nl.tile_size.pmax
    ...
    nisa.memset(dst=out_sb[...], value=0.0)
    nisa.memset(dst=out_quant_sb[...], value=0.0)
    nisa.memset(dst=out_scale_sb[...], value=0.0)
    ...
    for tile_t in nl.sequential_range(n_T256_tiles):
        ...
        for tile_i in nl.sequential_range(n_total_I512_tiles):
            cur_tile_I_size = min(512, I_local - tile_i * 512)
            cur_I_pdim_sz = cur_tile_I_size // 4
            out_psum = nl.ndarray((TILE_I, I_4, TILE_T), dtype=nl.bfloat16, buffer=nl.psum)
            cur_I128_tile_sz = cur_tile_I_size // 4
            for q_width_I_idx in nl.sequential_range(_q_width):
                weight_I_offset = tile_i * 512 + q_width_I_idx * cur_I128_tile_sz
            ...
            out_sb shape: [TILE_I, n_total_I512_tiles, T, I_4] = [128, 6, 32, 4]
            out_psum shape: [TILE_I, I_4, TILE_T] = [128, 4, 256]
            gate_bias_sb shape: [TILE_I, n_total_I512_tiles, I_4] = [128, 6, 4]
    ...
    for tile_t in nl.sequential_range(n_T256_tiles):
        ...
        for tile_i in nl.sequential_range(n_total_I512_tiles):
            ...
            out_psum shape: [TILE_I, I_4, TILE_T] = [128, 4, 256]
            up_bias_sb shape: [TILE_I, n_total_I512_tiles, I_4] = [128, 6, 4]
            Use strided access pattern to reorder from [TILE_I, I_4, TILE_T] to [TILE_I, TILE_T, I_4].
            Use cur_I_pdim_sz for actual partitions.
            """
            if up_bias_sb != None:
            ...
            qmx_I_pdim_sz = _pad_to_valid_qmx_partitions(cur_I_pdim_sz)
            nisa.quantize_mx(
