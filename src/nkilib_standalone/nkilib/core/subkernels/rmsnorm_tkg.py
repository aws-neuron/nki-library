...
def rmsnorm_tkg(
    ...
    Args:
        input (nl.ndarray): Input tensor if shape [B, S, H] when stored in HBM, [128, B×S, H//128] when stored in SBUF
        ...
        hidden_scale (float): 1/H. if specified, use this value to calculate mean; otherwise, use hidden parsed from input.
    ...
    if output_in_sbuf:
        ...
        if use_heap_memory:
            sharded_sbuf_result = sbm.alloc_heap(
                (H0, BxS, H1), dtype=input.dtype, buffer=nl.sbuf, name="rmsnorm_shared_sbuf"
            )
        else:
...
def rmsnorm_tkg_llama_impl(
    ...
    H1 = H // H0
    For LNC, input is split up to [B, S, lnc, H//lnc], and reshaped to [BxS, lnc, H0, H1//lnc].
    ...
    :param input ndarray: Tensor to perform RMSNorm on, which has shape [B, S, H].
    ...
    sbm.open_scope(name="rmsnorm_impl")
    ...
    if input_in_sbuf:
        ...
        input_sb = alloc_tensor(shape=(H0, bs_count, H1), dtype=input.dtype, name="rmsnorm_input")
    ...
    if not input_in_sbuf:
        ...
        input_sb_view = TensorView(input_sb).reshape_dim(dim=2, shape=[lnc, H2])
        nisa.dma_copy(
    ...
    gamma_view = (
        TensorView(gamma)
        .reshape_dim(dim=1, shape=[lnc, H0, H2])
        .flatten_dims(start_dim=0, end_dim=1)
        .permute(dims=[1, 0, 2])
    )

    # Reshape gamma buffer to match sharded layout
    gamma_sb_view = TensorView(gamma_sb).reshape_dim(dim=1, shape=[lnc, H2])
