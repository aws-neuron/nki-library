...
"""
This file contains an implementation of an attention kernel optimized for token-generation (small active sequence length).
It also contains dataclasses for specifying shapes, optimization flags, and intermediate buffers.
...
def attention_tkg(
    ...
    1. Flexible Tensor Placement:
      ...
      - Use this feature for perofrmance improvement when integrating this kernel into a larger kernel
    ...
    atp = _compute_tile_params(cfg, TC, q, active_blocks_table)
...
def _compute_tile_params(cfg: AttnTKGConfig, TC: TileConstants, q, active_blocks_table) -> AttnTileParams:
...
def _setup_block_kv_cache(
    ...
    active_blocks_sb, num_folds_per_batch = _load_and_reshape_active_blk_table(
        ...
        cfg.bs,
        sbm,
        (atp.bs_prg_id * atp.bs, (atp.bs_prg_id + 1) * atp.bs),
    )
...
def _load_position_ids_and_mask(
    ...
    if rope_pos_ids is None:
        ...
        nisa.dma_copy(rope_pos_ids_loaded, rope_pos_ids[atp.bs_prg_id, :])
    ...
    if not cfg.use_pos_id:
        ...
        kernel_assert(
            not atp.is_block_kv,
            "In-kernel cache mask generation (use_pos_id=False) is not supported when using block kv",
        )
        ...
        if atp.sprior_prg_id == atp.sprior_n_prgs - 1:
            _gen_mask(
                ...
                mask,
                bufs.mask_sb,
                sprior_prg_id=atp.sprior_prg_id,
...
def _perform_rope(
    ...
    nisa.dma_copy(inv_freqs_sb, inv_freqs)
    ...
    _apply_rope(q, cos, sin, bufs.q_sb, cfg, sbm=sbm)
    if cfg.k_out_in_sb or (atp.sprior_prg_id == atp.sprior_n_prgs - 1):
        _apply_rope(k_active, cos, sin, bufs.k_active_sb, cfg, ignore_heads=True, sbm=sbm)
...
def _compute_qk_matmul(
    ...
    per_batch_interleave_degree = math.floor(float(TC.psum_b_max) / atp.batch_interleave_degree)
    sbm.open_scope(interleave_degree=atp.batch_interleave_degree)
    for i_b in range(atp.bs):
        # Load entire K_prior for current batch into sbuf
        k_sb = sbm.alloc_stack((cfg.d_head, atp.s_prior), dtype=atp.io_type, buffer=nl.sbuf, align=32)
        if atp.is_block_kv:
            sbm.open_scope()
...
def _cascaded_max_reduce(
    ...
    for i_bsq_tile in range(atp.n_bsq_full_tiles):
        _transpose_max_psum(i_bsq_tile, atp.s_active_bqh_tile, atp, TC, bufs)

    if atp.s_active_bqh_remainder > 0:
        _transpose_max_psum(atp.n_bsq_full_tiles, atp.s_active_bqh_remainder, atp, TC, bufs)
    sbm.close_scope()
...
def _transpose_max_psum(
    ...
    qk_max_psum = nl.ndarray(
        ...
        dtype=atp.inter_type,
        buffer=nl.psum,
        address=(0, (index % TC.psum_b_max) * TC.psum_f_max_bytes),
    )
    nisa.nc_transpose(
        qk_max_psum,
...
def _transpose_broadcast_max(
    ...
    tp_broadcast(src=qk_max_buf_copy, dst=qk_max_copy, src_offset=0)
    nisa.tensor_copy(
        bufs.qk_max[:, nl.ds(index * atp.s_active_bqh_tile, tile_size)],
        qk_max_copy,
    )
...
def _compute_exp_qk(DBG_TENSORS, atp: AttnTileParams, TC: TileConstants, bufs: AttnInternalBuffers):
    ...
    qk_max_pat = bufs.qk_max.ap(
        pattern=[
            [atp.s_active_bqh, TC.p_max],
            [0, atp.n_sprior_tile],
            [1, atp.s_active_bqh],
        ],
        offset=0,
    ...
    nisa.tensor_tensor(
        qk_pat, qk_pat, qk_max_pat, op=(nl.add if atp.max_negated else nl.subtract)
    )  # rhs is broadcasted
...
def _cascaded_sum_reduction(
    ...
    sbm.open_scope()
    for i_bsq_tile in range(atp.n_bsq_full_tiles):
        _tile_sum_reduction(i_bsq_tile, atp.s_active_bqh_tile, atp, TC, bufs)
    sbm.close_scope()

    if atp.s_active_bqh_remainder > 0:
        sbm.open_scope()
        _tile_sum_reduction(atp.n_bsq_full_tiles, atp.s_active_bqh_remainder, atp, TC, bufs)
        sbm.close_scope()
...
def _tile_sum_reduction(index, tile_size, atp: AttnTileParams, TC: TileConstants, bufs: AttnInternalBuffers):
    """
    ...
    for i_exp_reduce in range(atp.n_sprior_tile):
        ...
        qk_io_offset = index * tile_size + i_exp_reduce * atp.s_active_bqh
...
def _tile_reciprocal_transpose(index, tile_size, atp: AttnTileParams, TC: TileConstants, bufs: AttnInternalBuffers):
    ...
    for i in range(div_ceil(tile_size, TC.sbuf_quadrant_size)):
        offset = i * TC.sbuf_quadrant_size
...
def _compute_pv_matmul_and_store(
    ...
    for i_b in range(atp.bs):
        ...
        if atp.is_block_kv:
            ...
            nisa.dma_copy(v_sb_pat, v_prior_pat)
        else:
            v_prior_offset = (
                i_b + atp.bs_prg_id * atp.bs
            ) * cfg.full_sprior * cfg.d_head + atp.sprior_prg_id * atp.s_prior * cfg.d_head
            ...
            nisa.dma_copy(dst=v_sb_pat, src=v_prior_pat, dge_mode=nki.isa.dge_mode.none)
        ...
        if atp.sprior_prg_id == atp.sprior_n_prgs - 1:
            if atp.is_block_kv:
                ...
                if load1_nrows > 0:
                    ...
                    nisa.dma_copy(v_sb_pat, v_active_pat)
                ...
                # Load to the bottom part of last chunk of v_sb: [p_max, d_head]
                nisa.dma_copy(
                    v_sb[TC.p_max - cfg.s_active :, v_sb.shape[1] - cfg.d_head :],
                    v_active[atp.bs_prg_id * atp.bs + i_b, 0, :, :],
                )
        ...
        exp_v_psum = nl.ndarray(
            ...
            dtype=nl.float32,
            buffer=nl.psum,
            address=(0, (i_b % atp.batch_interleave_degree) * TC.psum_f_max_bytes),
        )
    ...
    if cfg.out_in_sb:
        ...
        if atp.sprior_prg_id == 0:
            ...
            nisa.dma_copy(dst=out_pat, src=exp_v_reshaped)
...
def _get_lnc_sharding(cfg: AttnTKGConfig) -> Tuple[int, int, int, int]:
    ...
    if n_prgs > 1:
        ...
        elif cfg.curr_sprior >= 256:  # If s_prior is small, and batch is not divisible by lnc
...
def _apply_rope(
    ...
    if not cfg.qk_in_sb:
        ...
        nisa.dma_copy(x_pre_tp[...], x_inp)
        # FIXME: Add arch check
        tp_dtype = x_pre_tp.dtype
        x_tp_psum = nl.ndarray((d_head, x_f), dtype=tp_dtype, buffer=nl.psum, address=(0, 0))
...
### Mask Generation
def _gen_mask(pos_ids, active_mask, mask_sb, sprior_prg_id, strided_mm1: bool, sbm):
    """Generate prior mask with position IDs, then optionally load an active mask in the end.

    Args:
      pos_ids: SB input, shape [P_MAX, bs * s_active], P_MAX is broadcasted
      active_mask_hbm: HBM input, shape [s_active, bs, q_head, s_active]

    Returns:
      mask_sb: output mask, shape [P_MAX, n_sprior_tile, bs, q_head, s_active]
    """
    TILE_CONSTANTS = TileConstants.get_tile_constants()
    P_MAX = TILE_CONSTANTS.p_max

    nisa.memset(mask_sb, value=0)
    kernel_assert(
        len(mask_sb.shape) == 5,
        "_gen_mask expects a 5D tensor of shape (P_MAX, n_sprior_tile, bs, q_head, s_active). "
        "Allocate or reshape to a 5D tensor.",
    )
    _, n_sprior_tile, bs, q_head, s_active = mask_sb.shape
    s_prior, s_active_qh, s_active_bqh = (
        n_sprior_tile * P_MAX,
        q_head * s_active,
        bs * q_head * s_active,
    )

    # Create a tensor with row indices. If not strided mm1, par dim (s_prior) wraps around onto free (n_sprior_tile)
    tmp_iota = sbm.alloc_stack((P_MAX, n_sprior_tile), dtype=pos_ids.dtype, buffer=nl.sbuf)
    ...
    # Repeat mask_iota s_active_qh times for each elt
    mask_iota = sbm.alloc_stack((P_MAX, n_sprior_tile * s_active_qh), dtype=tmp_iota.dtype)
    nisa.tensor_copy(
        dst=mask_iota.ap(
            pattern=[
    ...
    # Create prior mask by per-batch tensor_scalar with corresponding pos_id
    # Use more memory to improve performance. If the batch size is very large,
    # a new scope needs to be opened here.
    for i_b in range(bs):
        cur_mask = sbm.alloc_stack(mask_iota.shape, dtype=mask_sb.dtype, buffer=nl.sbuf)
        print(
            f"The value of the offset is {i_b * s_active}, while the shape of the tensor is {pos_ids.shape}, the access is {pos_ids[:, i_b * s_active].offset}"
        )
        nisa.tensor_scalar(
            dst=cur_mask[...],
            data=mask_iota[...],
            op0=nl.less,
            operand0=pos_ids[:, nl.ds(i_b * s_active, 1)],
        )

        # Copy mask for this batch to mask_sb, where batch dim is interleaved on fdim (see layout for qk)
        cur_mask = cur_mask.reshape((P_MAX, n_sprior_tile, q_head, s_active))
        mask_sb_pat = mask_sb.ap(
            [
                [n_sprior_tile * bs * q_head * s_active, P_MAX],
                [bs * q_head * s_active, n_sprior_tile],
                [1, q_head * s_active],
            ],
            offset=i_b * q_head * s_active,
        )
        if i_b % 2 == 0:
            nisa.tensor_copy(mask_sb_pat, cur_mask[...], engine=nisa.scalar_engine)
        else:
            nisa.tensor_copy(mask_sb_pat, cur_mask[...], engine=nisa.vector_engine)

    # Optionally load active mask onto the last section of mask_sb
    if active_mask is not None:
        if strided_mm1:
            # Need to load active mask in a strided manner across the entire free dim, this requires two loads
            load1_nrows = s_active % n_sprior_tile
            load2_nrows = s_active - load1_nrows

            # Load 1. Load first portion of active mask onto one partition
            if load1_nrows > 0:
            ...
            # Load 2. Load remaining active mask onto the last few partitions
            if load2_nrows > 0:
            ...
            # Load to bottom right chunk of size [s_active, s_active_bqh]
            active_mask = active_mask.reshape((s_active, 1, s_active_bqh))
            nisa.dma_copy(mask_sb[P_MAX - s_active :, n_sprior_tile - 1 :, :], active_mask[...])
...
def _prep_sink(
    ...
    nisa.dma_copy(sink_sb, sink_hbm.reshape((1, cfg.q_head)))
...
def _load_and_reshape_active_blk_table(
    ...
    batch_size,
    sbm: SbufManager,
    batch_slicing: Optional[Tuple[int, int]] = None,
):
    """
  ...
  """
    if resize_factor == 1:
        ...
        active_blk_table_reshape = active_blk_table.reshape(
            (batch_size, num_128_folds_per_batch * n_prgs, partition_resize)
        )
        if batch_slicing is not None:
            active_blk_table_reshape = active_blk_table_reshape[batch_slicing[0] : batch_slicing[1], :, :]
            batch_size = batch_slicing[1] - batch_slicing[0]

        active_blk_table_sb = sbm.alloc_stack(
            (TC.p_max, batch_size * num_128_folds_per_batch),
            dtype=active_blk_table_reshape.dtype,
            buffer=nl.sbuf,
            align=4,
        )
        ...
        src_ap = [
            [1, partition_resize],
            [0, resize_factor],
            [partition_resize, num_128_folds_per_batch],
            [num_128_folds_per_batch * n_prgs * partition_resize, batch_size],
        ]
        src_offset = (prg_id * num_128_folds_per_batch) * partition_resize

        dst_ap = [
            [(num_128_folds_per_batch * batch_size) * resize_factor, partition_resize],
            [num_128_folds_per_batch * batch_size, resize_factor],
            [batch_size, num_128_folds_per_batch],
            [1, batch_size],
        ]

        nisa.dma_copy(
            src=active_blk_table_reshape.ap(src_ap, offset=src_offset),
            dst=active_blk_table_sb.ap(dst_ap),
        )
    else:
        if batch_slicing is not None:
            batch_slicing = (0, batch_size)
        ...
        nisa.dma_copy(
            ...
            src=active_blk_table[
                batch_slicing[0] : batch_slicing[1],
                prg_id * blks_per_prg : (prg_id + 1) * blks_per_prg,
            ],
        )
        ...
        sbm.open_scope()
        for i in range(num_128_folds_per_batch):
            active_blk_transposed = nl.ndarray(
                (TC.p_max, batch_size),
                dtype=active_blk_reshaped.dtype,
                buffer=nl.psum,
                address=(0, (i % TC.psum_b_max) * TC.psum_f_max_bytes),
            )
            nisa.nc_transpose(
                active_blk_transposed,
                active_blk_reshaped[:, nl.ds(i * TC.p_max, TC.p_max)],
            )
            nisa.tensor_copy(
                active_blk_table_sb[:, nl.ds(i * batch_size, batch_size)],
