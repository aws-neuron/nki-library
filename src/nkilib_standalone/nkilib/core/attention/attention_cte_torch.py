...
def attention_cte_torch_ref(
    ...
    cp_offset = 0 if cp_offset is None else int(cp_offset.item())
    mask = q.new_full((seqlen_q, prior_used_len + seqlen_k), 0)
    if causal_mask:
        mask += torch.triu(
            q.new_full((seqlen_q, prior_used_len + seqlen_k), -float("inf")),
            diagonal=prior_used_len + 1 + cp_offset,
        )
    if sliding_window > 0:
        mask += torch.tril(
            mask.new_full((seqlen_q, prior_used_len + seqlen_k), -float("inf")),
            diagonal=prior_used_len - sliding_window + cp_offset,
        )
