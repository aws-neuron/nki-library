...
def attention_cte(
    ...
    Args:
      ...
      tp_q (bool): Query tensor transpose flag
      tp_k (bool): Key tensor transpose flag
      tp_out (bool): Output tensor transpose flag
      cache_softmax (bool): Whether to cache softmax intermediate values
    ...
    Context Parallel Support:
      ...
      Note: cp_offset := rank_id * partial_q_seqlen.
    ...
    if k_prior != None:
        ...
        kernel_assert(v_prior != None, "k_prior != None but v_prior is None for prefix caching")
        kernel_assert(
            prior_used_len != None,
            "k_prior != None but prior_used_len is None for prefix caching",
        )
    else:
        is_prefix_caching = False
        kernel_assert(v_prior == None, "k_prior is None but v_prior is not None.")
        kernel_assert(prior_used_len == None, "k_prior is None but prior_used_len is not None.")
    ...
    if has_remainder:
        ...
        if seqlen_q >= _MIN_SEQLEN_FOR_LNC2_SHARDING:
            ...
            use_causal_divide_factor = causal_mask and (cp_offset == None) and (sliding_window == 0)
            divide_factor = (
                _SEQLEN_SHARDING_SPLIT_FACTOR_CAUSAL
...
class AttnTileParams(nl.NKIObject):
    ...
    is_causal: bool = None  # generally same as causal_mask, but modified in CP case
...
def _compute_tile_parameters(
    ...
    # When use CP or SWA, tiles are dynamically masked (mask unknown at compile time), so we turn off causal
    # to disable compute-skipping
    # NOTE: we technically can skip some compute with SWA, postponing this work due to little perf gain (KV seqlen
    # already short under SWA)
    ...
    if ac.use_swa:
        # When using CP (dynamic sbuf CP offsets), we (1) do dynamic masking with range_selects and (2) load reduced KV
        ...
        if ac.use_cp:
            atp.is_causal = False
            # similar to CP attn, SWA also uses lb and ub to mask mm1 output
            atp.dynamic_sel_mask = True
    ...
    atp.num_q_grps_per_load = min(8 if atp.can_pack_q_load else 1, atp.num_grps)
    kernel_assert(
...
def _setup_range_select_bounds(
    ...
    if atp.dynamic_sel_mask:
        ...
        if ac.use_cp:
            ...
            kernel_assert((cp_offset != None), "cp_offset missing but global_cp_deg is provided")
            kernel_assert(
                (cp_offset.shape == (1, 1)),
...
def _allocate_attention_buffers(
    ...
    mm1_partial_max_n_elts = atp.num_k_tiles_per_section + (sink != None)
    ...
    n_final_reduce_sum_elts = math.ceil(atp.section_len / atp.exp_inst_elems) + (sink != None)
...
def _load_k_tile(
    ...
    seqlen_prior = None
    is_prefix_caching = k_prior != None
    if is_prefix_caching:
        if tp_k:
    ...
    if tp_k:
        ...
        for tile in range(num_tiles):
            ...
            use_dma_tp = load_offset == None  # cannot use dma tp when using dynamic offset
            if use_dma_tp:
...
def _load_v_tile(
    ...
    seqlen_prior = None
    is_prefix_caching = v_prior != None
    if is_prefix_caching:
...
def _load_q_impl(
    ...
    if grp_i % atp.num_q_grps_per_load == 0:
        has_any_compute_pred = (
            (grp_i * _Q_GRP_SZ + _Q_GRP_SZ * atp.num_q_grps_per_load) > sp.section_offset_active
...
def _qk_and_max_impl(
    ...
    """
    has_any_compute_pred = (
        (grp_i * _Q_GRP_SZ + _Q_GRP_SZ) > sp.section_offset_active
        if (atp.is_causal and not sp.section_contains_prefix)
        else True
    )
    if has_any_compute_pred:
...
def _update_max_impl(
    ...
    if (sink != None) and (sp.section_idx == 0):
    ...
    for large_tile_idx in range(atp.num_large_tiles_per_section):
        ...
        for exp_tile_idx in range(atp.num_exp_insts_per_large_tile):
            ...
            if atp.is_causal and not is_prior_tile:
                exp_sel_mask = q_start_pos + _Q_GRP_SZ > k_start_pos
            else:
                exp_sel_mask = True
            ...
            if ac.use_swa and atp.is_causal and not is_prior_tile:
                ...
                exp_sel_mask = exp_sel_mask and (q_start_pos - ac.sliding_window < k_start_pos + atp.exp_inst_elems)
...
def _fused_qkmax_and_pv_impl(
    ...
    has_any_compute_pred_qkmax = (
        (qkmax_grp * _Q_GRP_SZ + _Q_GRP_SZ) > sp.section_offset_active
        if (atp.is_causal and not sp.section_contains_prefix)
        else True
    )
...
def _write_back_impl(
    ...
    next_has_compute_pred = (
        ((grp_i * _Q_GRP_SZ + _Q_GRP_SZ) > sp.next_section_offset_active)
...
def _qk_and_max_large_tile_impl(
    ...
    for k_tile_idx in range(num_k_tiles_in_large_tile):
        ...
        if atp.is_causal and not is_prior_tile:
            q_start_pos = qkmax_grp * _Q_GRP_SZ
            # Only produce matmul if the tile is in the lower triangle, use tile bot-left corner so adjust q
            matmul_selection = q_start_pos + _Q_GRP_SZ > k_start_pos
            # If using SWA, also skip bot-left lower triangle.
            if ac.use_swa:
                # Use tile top-right corner so adjust k; also adjust q for sliding window
                matmul_selection = matmul_selection and (q_start_pos - ac.sliding_window < k_start_pos + _K_TILE_SZ)
        else:
...
def _pv_large_tile_impl(
    ...
    for mm2_grp_i in range(num_mm2_grps_in_large_tile):
        ...
        for mm2_i in range(num_mm2_per_grp):
            ...
            # Only produce matmul if the tile is in the lower triangle, use tile bot-left corner so adjust q
            mm2_sel_mask = (q_start_pos + _Q_GRP_SZ > k_start_pos) if (atp.is_causal and not is_prior_tile) else True
            # If using SWA, also skip bot-left lower triangle.
            if ac.use_swa and atp.is_causal and not is_prior_tile:
                # Use tile top-right corner so adjust k; also adjust q for sliding window
                mm2_sel_mask = mm2_sel_mask and (q_start_pos - ac.sliding_window < k_start_pos + _V_TILE_SZ)
    ...
    if is_prior_tile:
        kernel_assert(
            k_seqlen_mask is True, "Internal validation failed."
        )  # if first 512 tile is prior, we must have k_start_pos<seqlen_k since prior is only padded to 512 max

    q_start_pos = pv_grp * _Q_GRP_SZ
    # Only produce matmul if the tile is in the lower triangle, use tile bot-left corner so adjust q
    loop_reduce_sel = (q_start_pos + _Q_GRP_SZ > k_start_pos) if (atp.is_causal and not is_prior_tile) else True
    # If using SWA, also skip bot-left lower triangle.
    if ac.use_swa and atp.is_causal and not is_prior_tile:
        # Use tile top-right corner so adjust k; also adjust q for sliding window
        loop_reduce_sel = loop_reduce_sel and (q_start_pos - ac.sliding_window < k_start_pos + _LARGE_TILE_SZ)
