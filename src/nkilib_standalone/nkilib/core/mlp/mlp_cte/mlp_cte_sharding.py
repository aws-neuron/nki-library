...
def _calculate_bxs_sharding(
    ...
    kernel_assert(
        bxs % bxs_shard_size == 0,
        f"VNC-sharded MLP kernel requires bxs to be divisible by {bxs_shard_size} "
        f"but got {mlp_params.batch_size} * {mlp_params.sequence_len}",
    )
    num_shards = bxs // bxs_shard_size
    kernel_assert(
        num_shards % num_shard_workers == 0,
        f"Expected number of shards to be evenly shardable between {num_shard_workers} "
        f"cores but got {num_shards} tiles",
    )
    num_shards_per_worker = num_shards // num_shard_workers

    shard_list = []
    for tile_idx in range(num_shards_per_worker):
        bxs_offset = (program_id * num_shards_per_worker + tile_idx) * bxs_shard_size
        shard_list.append(DimShard(bxs_offset, bxs_shard_size, mlp_params))
